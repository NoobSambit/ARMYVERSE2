    ...
    <div className="flex-grow w-full max-w-[1600px] mx-auto p-3 sm:p-4 lg:p-8 flex flex-col gap-4 sm:gap-6 font-display">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 mb-1 sm:mb-2">
        <div className="flex flex-col gap-1.5 sm:gap-2">
          <h1 className="text-white text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight">Create Playlist</h1>
          <p className="text-text-muted text-xs sm:text-sm md:text-base">Manually curate your perfect setlist or generate one with AI.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 items-start relative">
          ...
          <section className="lg:col-span-7 xl:col-span-8 flex flex-col gap-3 sm:gap-4">
            ...
            <div className="grid grid-cols-4 gap-1.5 sm:gap-3">
              <div className="bg-surface-light/50 border border-border-dark p-2 sm:p-3 rounded-xl flex flex-col items-center justify-center text-center">
                <span className="text-[9px] sm:text-xs font-semibold text-text-muted uppercase tracking-wider">Tracks</span>
                <span className="text-base sm:text-xl font-bold text-white">{playlistStats.tracks}</span>
              </div>
              <div className="bg-surface-light/50 border border-border-dark p-2 sm:p-3 rounded-xl flex flex-col items-center justify-center text-center">
                <span className="text-[9px] sm:text-xs font-semibold text-text-muted uppercase tracking-wider">Duration</span>
                <span className="text-base sm:text-xl font-bold text-white text-xs sm:text-sm">{playlistStats.duration}</span>
              </div>
              <div className="bg-surface-light/50 border border-border-dark p-2 sm:p-3 rounded-xl flex flex-col items-center justify-center text-center">
                <span className="text-[9px] sm:text-xs font-semibold text-text-muted uppercase tracking-wider">Appearances</span>
                <span className="text-base sm:text-xl font-bold text-white">{playlistStats.appearances}</span>
              </div>
              <div className="bg-surface-light/50 border border-border-dark p-2 sm:p-3 rounded-xl flex flex-col items-center justify-center text-center">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="text-white font-bold truncate text-sm">{track.name}</h4>
                            <p className="text-text-muted text-xs sm:text-sm truncate">{track.artist} • {track.album}</p>
                          </div>
                          <button
                  ...
                  <>
                    {selectedTracks.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center py-8 sm:py-12 border-2 border-dashed border-white/5 rounded-xl">
                        ...
                        <p className="text-text-muted text-sm">No tracks selected</p>
                        <p className="text-gray-600 text-xs sm:text-sm mt-1">Search and add tracks above</p>
                      </div>
                    ) : (
                      selectedTracks.map((track) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1.5 sm:gap-3 bg-background-dark/50 p-1 sm:p-1.5 rounded-xl border border-white/5">
                            ...
                            <span className="w-3 sm:w-4 text-center text-xs sm:text-sm font-bold text-white">{track.appearances}</span>
                            <button
                              onClick={() => updateAppearances(track.spotifyId, 1)}
          ...
          <aside className="lg:col-span-5 xl:col-span-4 sticky top-4 sm:top-6 self-start z-10">
            <div className="bg-surface-dark/70 backdrop-blur-xl border border-glass-border rounded-2xl p-3 sm:p-5 flex flex-col gap-3 sm:gap-5 h-[calc(100vh-4rem)] sm:h-[calc(100vh-6rem)] min-h-[400px] sm:min-h-[500px]">
              ...
              <div className="flex flex-col gap-2 sm:gap-3 pb-3 sm:pb-4 border-b border-white/5">
                ...
                <div className="flex gap-2 sm:gap-4">
                  <div className="flex-1 flex flex-col gap-1">
                    <span className="text-[9px] sm:text-[10px] uppercase font-bold text-text-muted tracking-wider">Shuffle Quality</span>
                    ...
                    <span className="text-[10px] sm:text-xs font-bold text-white text-right">{playlistQuality.shuffleQuality}%</span>
                  </div>
                  <div className="flex-1 flex flex-col gap-1">
              ...
              {/* Playlist Name Input */}
              <div>
                <label className="text-[10px] sm:text-xs font-bold text-text-muted uppercase tracking-wider mb-1.5 sm:mb-2 block">Playlist Name</label>
              ...
              <div className="flex-1 overflow-y-auto pr-1 space-y-1.5 sm:space-y-2 custom-scrollbar">
                {playlistTracks.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center py-8 sm:py-12">
                    ...
                    <p className="text-text-muted text-xs sm:text-sm">No playlist generated yet</p>
                    <p className="text-gray-600 text-[10px] sm:text-xs mt-1">Add tracks and click Update Preview</p>
                  </div>
                ) : (
                  playlistTracks.map((track, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-xs sm:text-sm font-semibold truncate">{track.name}</p>
                      ...
                      >
                        <svg className="w-[14px] sm:w-[18px] h-[14px] sm:h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {playlistTracks.length > 0 && (
                <div className="pt-3 sm:pt-4 mt-auto border-t border-white/5 flex flex-col gap-2 sm:gap-3">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <span className="text-[10px] sm:text-xs text-text-muted">Account:</span>
