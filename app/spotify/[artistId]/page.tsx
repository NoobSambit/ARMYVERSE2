...
const normalizeAllTimeStreams = (value?: number) => {
  if (!value) return 0
  // Some snapshots store kworb artist streams in millions; normalize to absolute.
  return value < 1_000_000 ? value * 1_000_000 : value
}

const matchesArtist = (a?: string, b?: string) => {
  if (!a || !b) return false
...
export default function ArtistPage() {
  ...
  const artistSongs = snap.songsByArtist?.find(g =>
    matchesArtist(g.artist, artistName)
  )
  const artistAlbums = snap.albumsByArtist?.find(g =>
    matchesArtist(g.artist, artistName)
  )
  const artistAllTime = snap.artistsAllTime?.find(r =>
    matchesArtist(r.artist, artistName)
  )
  const artistMonthly = snap.monthlyListeners?.find(
    r => matchesArtist(r.artist, artistName)
  )
  const artistMeta = (snap.artistMetadata as any)?.[artistName] || {}

  // Calculate Stats
  const totalStreams = Math.max(
    normalizeAllTimeStreams(artistAllTime?.streams),
    artistSongs?.totals.streams || 0
  )
  ...
  // Changes
  const streamsChange =
    data.changes24h?.songsByArtist?.[artistName]?.streamsChange ??
    (artistSongs?.totals.daily || 0)
  const streamsPercent = totalStreams
    ? (streamsChange / totalStreams) * 100
    : 0

  const listenersChange = artistMonthly?.dailyChange || 0
  const listenersPercent = monthlyListeners
    ? (listenersChange / monthlyListeners) * 100
    : 0
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
              ...
              <ArtistStatsRow
                stats={{
                  streams: {
                    ...
                    change: parseFloat(streamsPercent.toFixed(2)),
                    percent: 85,
                  },
                  listeners: {
                    value: formatNumber(monthlyListeners),
                    change: parseFloat(listenersPercent.toFixed(2)),
                    percent: 65,
                  },
                  followers: {
          ...
          <TopSongs
            songs={(artistSongs?.songs || [])
              ...
              .map(s => ({
                ...
                spotifyId: s.spotifyId || extractSpotifyTrackId(s.url),
              }))}
              ...
              type:
                a.albumType === 'single'
                  ...
                  : a.albumType === 'compilation'
                    ...
                    : 'Album',
              year: a.releaseDate
                ? new Date(a.releaseDate).getFullYear().toString()
                : '2023',
        ...
        <div className="hidden xl:block xl:col-span-1">
          <ArtistSidebar
            ...
            similar={[
              { name: 'RM', imageUrl: '/placeholder-rm.jpg' },
              { name: 'Jin', imageUrl: '/placeholder-jin.jpg' },
              { name: 'Agust D', imageUrl: '/placeholder-agustd.jpg' },
              { name: 'J-Hope', imageUrl: '/placeholder-jhope.jpg' },
              { name: 'Jimin', imageUrl: '/placeholder-jimin.jpg' },
              { name: 'V', imageUrl: '/placeholder-v.jpg' },
              { name: 'Jungkook', imageUrl: '/placeholder-jk.jpg' },
            ]}
          />
        </div>
