        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {loading ? (
            <div className="h-full sm:h-96 flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-purple-200/30 font-medium">
            ...
            <div className="p-4 sm:p-6 md:p-8 space-y-6 sm:space-y-8 pb-20 sm:pb-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                <div className="p-3 sm:p-5 rounded-[16px] sm:rounded-[24px] bg-purple-500/5 border border-purple-500/10">
                  <div className="flex items-center gap-1.5 sm:gap-2 mb-1 sm:mb-2">
                    ...
                    <span className="text-[10px] sm:text-xs font-medium text-purple-200/40 uppercase tracking-wider">
                      Views
                    </span>
                  </div>
                  <p className="text-lg sm:text-2xl font-bold text-white">
                    {formatNumber(videoDetail.totalViews)}
                  </p>
                </div>
                ...
                <div className="p-3 sm:p-5 rounded-[16px] sm:rounded-[24px] bg-purple-500/5 border border-purple-500/10">
                  ...
                  <p className="text-sm sm:text-lg font-bold text-white truncate">
                    {formatDate(videoDetail.published)}
                  </p>
                </div>
              </div>
              ...
              {videoDetail.expectedMilestone && (
                <div className="p-4 sm:p-6 rounded-[20px] sm:rounded-[24px] bg-gradient-to-r from-purple-600/20 to-fuchsia-600/20 border border-purple-500/20 flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm text-purple-200/60 font-medium">
                      Next Milestone
                    </p>
                    <p className="text-sm sm:text-base text-white font-semibold leading-tight">
                      <span className="text-white font-bold">
                        {formatNumber(videoDetail.milestoneViews)}
                      </span>{' '}
                      views on {formatDate(videoDetail.milestoneDate)}
                    </p>
                  </div>
                </div>
              )}
              ...
              {(videoDetail.peakPosition > 0 ||
                ...
                <div className="p-4 sm:p-6 rounded-[20px] sm:rounded-[24px] bg-[#151515] border border-purple-500/10">
                  ...
                  <div className="flex gap-8 sm:gap-12">
                    {videoDetail.peakPosition > 0 && (
                      <div>
                        <p className="text-[10px] sm:text-xs text-white/40 font-medium uppercase tracking-wider mb-0.5 sm:mb-1">
                          Peak
                        </p>
                        <p className="text-2xl sm:text-3xl font-bold text-white">
                          #{videoDetail.peakPosition}
                        </p>
                      </div>
                    )}
                    {videoDetail.chartedWeeks > 0 && (
              ...
              <div>
                <div className="flex gap-1 sm:gap-2 mb-4 sm:mb-6 border-b border-purple-500/10 pb-1 overflow-x-auto scrollbar-hide">
                  {(['overview', 'daily', 'monthly', 'yearly'] as const).map(
                    tab => (
                      <button
                        key={tab}
                ...
                <div className="min-h-[250px] sm:min-h-[300px]">
                  {activeTab === 'overview' && (
                    <div className="grid gap-4">
                      {/* Top Lists */}
                      {videoDetail.topLists &&
                        videoDetail.topLists.length > 0 && (
                          <div className="p-4 sm:p-6 rounded-[20px] sm:rounded-[24px] bg-[#151515] border border-purple-500/10">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {videoDetail.topLists.map((list, idx) => (
                                <span
                                  key={idx}
                                  className="px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs font-medium bg-purple-500/10 text-purple-200 border border-purple-500/20"
                                >
                                  {list}
                                </span>
                              ))}
                      ...
                      {/* Milestones */}
                      {videoDetail.milestones &&
                        videoDetail.milestones.length > 0 && (
                          <div className="p-4 sm:p-6 rounded-[20px] sm:rounded-[24px] bg-[#151515] border border-purple-500/10">
                            <h4 className="font-bold text-white mb-3 sm:mb-4 flex items-center gap-2 text-sm sm:text-base">
                      ...
                      <div className="space-y-3 sm:space-y-4 max-h-[300px] sm:max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                        {videoDetail.dailyViews
                          ...
                          .map((day, idx) => (
                            ...
                            >
                              <span className="text-[10px] sm:text-xs text-white/40 w-16 sm:w-24 shrink-0 font-mono">
                                {day.date}
                              </span>
