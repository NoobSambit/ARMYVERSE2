...
export default function BTSAnalytics({ btsAnalytics, loading = false }: BTSAnalyticsProps) {
  ...
  if (loading) {
    return (
      <motion.div
        ...
        className="bg-black/20 backdrop-blur-xl rounded-3xl p-6 border border-white/5 animate-pulse min-h-[400px]"
      >
      </motion.div>
    )
  }
  ...
  return (
    <motion.div
      ...
      className="bg-black/20 backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-white/5 relative overflow-hidden"
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold text-white">BTS Analytics</h3>
            <p className="text-xs text-gray-400">Deep dive into your fandom</p>
          </div>
        </div>
        ...
        {viewMode === 'overview' && (
          ...
          >
            ...
            <div className="bg-gradient-to-r from-purple-900/40 to-indigo-900/40 border border-purple-500/20 rounded-2xl p-6 flex items-start gap-4">
               ...
               <div>
                 <h4 className="text-white font-bold mb-1">Super Fan Status</h4>
            ...
            <div>
               <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4">Member Breakdown</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {btsAnalytics.btsTracks.length > 0 ? (
                   <div className="bg-white/5 rounded-xl p-4 border border-white/5 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                         <p className="text-xs text-purple-400 font-bold uppercase mb-1">Top Track</p>
                         <p className="text-white font-bold truncate">{btsAnalytics.btsTracks[0]?.name || 'N/A'}</p>
                         <p className="text-gray-400 text-sm truncate">{btsAnalytics.btsTracks[0]?.album?.name || 'Unknown Album'}</p>
                      </div>
                   </div>
                 ) : (
                   <div className="bg-white/5 rounded-xl p-4 border border-white/5 flex items-center gap-4 justify-center">
                      <p className="text-gray-500 text-sm">No top track data available</p>
                   </div>
                 )}
                 ...
                 <div className="bg-white/5 rounded-xl p-4 border border-white/5 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                       <p className="text-xs text-purple-400 font-bold uppercase mb-1">Top Album</p>
                       <p className="text-white font-bold truncate">{btsAnalytics.favoriteBTSAlbum || 'N/A'}</p>
                    </div>
                 </div>
            </div>
          ...
          >
             <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
               {btsAnalytics.btsTracks.map((track, i) => {
                  ...
                  return (
                  <div key={track.id || i} className="flex items-center gap-4 p-3 hover:bg-white/5 rounded-xl transition-colors group">
                     ...
                     />
                     <div className="flex-1 min-w-0">
                        <p className="text-white font-medium truncate group-hover:text-purple-400 transition-colors">{track.name}</p>
