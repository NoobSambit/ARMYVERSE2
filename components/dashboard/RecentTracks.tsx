...
export default function RecentTracks({ tracks, loading = false }: RecentTracksProps) {
  ...
  if (loading) {
    return (
      <motion.div
        ...
        className="bg-black/40 backdrop-blur-xl rounded-2xl md:rounded-3xl p-4 md:p-6 border border-white/5 h-full min-h-[300px] md:min-h-[500px]"
      >
        <div className="animate-pulse">
          <div className="h-5 md:h-6 bg-gray-800 rounded mb-4 md:mb-6 w-1/3"></div>
          <div className="space-y-3 md:space-y-4">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="flex items-center space-x-3 md:space-x-4">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-gray-800 rounded-xl"></div>
                <div className="flex-1">
                  <div className="h-3.5 md:h-4 bg-gray-800 rounded mb-1.5 md:mb-2 w-3/4"></div>
                  <div className="h-2.5 md:h-3 bg-gray-800 rounded w-1/2"></div>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <motion.div
      ...
      className="bg-black/20 backdrop-blur-xl rounded-2xl md:rounded-3xl p-4 md:p-5 border border-white/5 h-full flex flex-col"
    >
      <div className="flex items-center justify-between mb-4 md:mb-6">
        <div className="space-y-0.5 md:space-y-1">
          <h3 className="text-base md:text-lg lg:text-xl font-bold text-white">Recent Tracks</h3>
          <p className="text-[10px] md:text-xs text-gray-400">Live updates from Last.fm</p>
        </div>
      </div>

      {/* Scrollable list container - responsive height */}
      <div className="flex-1 overflow-y-auto pr-1 md:pr-2 space-y-2 custom-scrollbar max-h-[40vh] md:max-h-[50vh] lg:max-h-[calc(100vh-250px)] min-h-[250px] md:min-h-[400px]">
          ...
          {tracks.map((track, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group flex items-center gap-2.5 md:gap-3 p-2.5 md:p-3 rounded-xl transition-all duration-300 ${
                  isNowPlaying
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-1.5 md:gap-2">
                    <p className={`font-semibold text-xs md:text-sm truncate ${isNowPlaying ? 'text-purple-300' : 'text-white'}`}>
                      {track.name}
                    </p>
                    {isNowPlaying && (
                       <Radio className="w-3 h-3 text-purple-400 animate-pulse shrink-0 mt-0.5" />
                    )}
                  </div>
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  {!isNowPlaying && (
                    <span className="text-[9px] md:text-[10px] text-gray-500 font-mono">{timeAgo}</span>
                  )}
                  {/* Always show external link on mobile, hover on desktop */}
                  <button
                    onClick={() => window.open(track.url || track.external_urls?.lastfm, '_blank')}
                    className="text-gray-500 hover:text-white transition-opacity opacity-100 md:opacity-0 md:group-hover:opacity-100"
                  >
      ...
      {tracks.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center text-center text-gray-500">
          <Music className="w-8 h-8 md:w-10 md:h-10 mb-2 opacity-20" />
          <p className="text-xs md:text-sm">No recent tracks</p>
        </div>
      )}
    </motion.div>
  )
