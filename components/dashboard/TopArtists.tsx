...
export default function TopArtists({ artists, loading = false }: TopArtistsProps) {
  ...
  if (loading) {
    return (
      <div className="bg-black/20 backdrop-blur-xl rounded-2xl md:rounded-3xl p-4 md:p-6 border border-white/5 animate-pulse min-h-[300px] md:min-h-[400px]">
      </div>
    )
  }
  ...
  return (
    <motion.div
      ...
      className="bg-black/20 backdrop-blur-xl rounded-2xl md:rounded-3xl p-4 md:p-6 lg:p-8 border border-white/5"
    >
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 md:mb-6 gap-3 md:gap-4">
        <div className="flex items-center gap-2.5 md:gap-3">
           <div className="bg-gradient-to-br from-purple-500/20 to-blue-500/20 p-1.5 md:p-2 rounded-xl border border-white/5">
              <BarChart2 className="w-5 h-5 md:w-6 md:h-6 text-purple-400" />
           </div>
           <div>
             <h3 className="text-base md:text-lg lg:text-xl font-bold text-white">Top Artists</h3>
             <p className="text-[10px] md:text-xs text-gray-400">Your most played artists</p>
           </div>
        </div>
            ...
            {artists.slice(0, 10).map((artist, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="group flex items-center gap-2.5 md:gap-4 p-2.5 md:p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-all duration-300"
                >
                  <div className="flex-shrink-0 w-6 md:w-8 text-center font-bold text-gray-500 group-hover:text-purple-400 transition-colors text-sm md:text-base">
                    #{rank}
                  </div>
                  ...
                  <div className="relative">
                    <img
                      src={imageUrl}
                      alt={artist.name}
                      className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover border-2 border-white/5 group-hover:border-purple-500/50 transition-colors"
                      onError={(e) => {
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-white font-medium text-sm md:text-base truncate">{artist.name}</p>
                    <p className="text-purple-400 text-[10px] md:text-xs font-bold">{formatNumber(playcount)} <span className="text-gray-500 font-normal">plays</span></p>
                  </div>

                  <button
                    onClick={() => window.open(artist.url || artist.external_urls?.lastfm, '_blank')}
                    className="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-white transition-opacity p-1.5 md:p-2"
                  >
          ...
          >
            <div className="h-56 md:h-72 lg:h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ bottom: 10, left: -10 }}>
                  ...
                  <XAxis
                    ...
                    interval={0}
                  />
                  <YAxis stroke="#9CA3AF" fontSize={10} tickFormatter={formatNumber} width={50} />
                  <Tooltip
                    contentStyle={{
