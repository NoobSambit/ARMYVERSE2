    ...
    <div className="text-white">
      ...
      <div className="mb-6 sm:mb-8">
        ...
        {selectedPrimaryTrack ? (
          <div className="mb-3 p-3 sm:p-4 bg-green-500/10 border border-green-500/30 rounded-xl sm:rounded-2xl flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="text-gray-400 hover:text-white text-xs sm:text-sm px-2 sm:px-3 py-1 hover:bg-white/10 rounded-xl transition-colors flex-shrink-0"
            >
              Change
            </button>
          </div>
        ...
        <input aria-label="Primary search" title="Primary search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search BTS songs…" className="w-full p-2.5 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-2xl mb-2 text-sm" />
        <div className="max-h-40 sm:max-h-48 overflow-y-auto space-y-1 custom-scrollbar">
          ...
          {primaryOptions.slice(0, 20).map(t => (
            <button key={t.spotifyId} onClick={() => { setPrimaryId(t.spotifyId); setSearch(''); }} className={`flex items-center w-full p-2 rounded-xl hover:bg-white/10 ${primaryId === t.spotifyId ? 'bg-purple-600/30' : ''}`}>
      ...
      <div className="mb-4 sm:mb-6 flex flex-wrap gap-2 sm:gap-4 items-center">
        ...
        <button onClick={() => setGapMode('auto')} className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm ${gapMode === 'auto' ? 'bg-purple-600' : 'bg-gray-700 hover:bg-gray-600'}`}>Auto</button>
        <button onClick={() => setGapMode('manual')} className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm ${gapMode === 'manual' ? 'bg-purple-600' : 'bg-gray-700 hover:bg-gray-600'}`}>Manual</button>
      </div>
      ...
      {gapMode === 'auto' ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div>
              ...
              <input aria-label="Min gap" title="Min gap" type="number" min={1} value={minGap} onChange={e => setMinGap(Number(e.target.value))} className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-xl text-sm" />
            </div>
            <div>
              <label className="block text-xs sm:text-sm mb-1 sm:mb-2 text-gray-300">Max gap</label>
              <input aria-label="Max gap" title="Max gap" type="number" min={minGap} value={maxGap} onChange={e => setMaxGap(Number(e.target.value))} className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-xl text-sm" />
            </div>
          </div>
          ...
          <div className="mb-4 sm:mb-6">
            ...
            <select aria-label="Fill mode" title="Fill mode" value={fillMode} onChange={e => setFillMode(e.target.value as FillMode)} className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-xl text-sm">
          ...
          {fillMode === 'album' && (
            <div className="mb-4 sm:mb-6">
              ...
              <input value={albumSearch} onChange={e=>setAlbumSearch(e.target.value)} placeholder="Search albums…" className="w-full p-2 mb-2 bg-gray-800/50 border border-gray-600 rounded-xl text-sm" />
          ...
          {fillMode === 'era' && (
            <div className="mb-4 sm:mb-6">
              ...
              <select aria-label="Era" title="Era" value={selectedEra} onChange={e => setSelectedEra(e.target.value)} className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-xl text-sm">
        ...
        <>
          ...
          <div className="mb-4 sm:mb-6">
            ...
            <input aria-label="Gap songs count" title="Gap songs count" type="number" min={1} value={gapCount} onChange={e => setGapCount(Number(e.target.value))} className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-xl text-sm" />
          </div>
          <div className="mb-4 sm:mb-6">
            <label className="block text-xs sm:text-sm mb-1 sm:mb-2 text-gray-300">Search & select gap songs</label>
            <input aria-label="Manual search" title="Manual search" value={manualSearch} onChange={e => setManualSearch(e.target.value)} placeholder="Search…" className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-xl mb-2 text-sm" />
            <div className="max-h-40 sm:max-h-48 overflow-y-auto space-y-1 custom-scrollbar">
              {manualFiltered.slice(0, 30).map(t => (
                <label key={t.spotifyId} className="flex items-center w-full p-2 rounded-xl hover:bg-white/10 cursor-pointer">
      ...
      <div className="mb-4 sm:mb-6">
        ...
        <input aria-label="Playlist length" title="Playlist length" type="number" min={5} value={totalLength} onChange={e => setTotalLength(Number(e.target.value))} className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-600 rounded-xl sm:rounded-xl text-sm" />
      </div>

      <button disabled={!primaryId || generating} onClick={handleGenerate} className={`w-full py-2.5 sm:py-3 rounded-2xl font-semibold flex items-center justify-center transition-all duration-300 text-sm sm:text-base ${primaryId ? 'bg-purple-600/80 hover:bg-purple-600' : 'bg-gray-600'} text-white`}>
