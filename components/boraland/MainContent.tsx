...
export default function MainContent({ onTabChange }: { pool: PoolInfo | null, onTabChange?: (tab: Tab) => void }) {
  return (
    <section className="flex-grow flex flex-col gap-6">
      {/* Banner */}
      <div data-tour="main-banner" className="relative w-full h-[280px] md:h-[400px] rounded-3xl overflow-hidden group shadow-2xl shadow-purple-900/20 border border-white/10">
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 md:p-8 lg:p-10 flex flex-col items-start z-10">
          <div className="inline-flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-0.5 md:py-1 rounded-full bg-accent-pink/20 border border-accent-pink/40 text-accent-pink text-[10px] md:text-xs font-bold uppercase tracking-wider mb-2 md:mb-4 animate-pulse">
            <span className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-accent-pink"></span> Live Event
          </div>
          <h1 className="font-display text-2xl md:text-4xl lg:text-5xl font-bold text-white mb-1 md:mb-2 leading-tight">
          ...
          <p className="text-gray-300 max-w-xl text-xs md:text-base lg:text-lg mb-3 md:mb-6 lg:mb-8 leading-relaxed hidden sm:block">
            Test your knowledge on BTS eras, songs, and moments. Collect exclusive digital photocards for perfect scores.
          </p>
          <div className="flex items-center gap-2 md:gap-4 w-full">
            <Link href="/boraland/play" className="flex-1 md:flex-none bg-white text-black hover:bg-gray-100 font-bold text-sm md:text-base lg:text-lg px-4 md:px-6 lg:px-8 py-2 md:py-3 lg:py-4 rounded-xl shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-[0_0_30px_rgba(255,255,255,0.5)] transition-all flex items-center justify-center gap-2 group/btn">
              <span className="material-symbols-outlined group-hover/btn:rotate-12 transition-transform text-base md:text-xl">play_arrow</span>
              Start Quiz
            </Link>
      ...
      {/* Cards */}
      <div data-tour="game-modes" className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
      ...
      <div data-tour="feature-cards" className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        <button
