...
export default function RightSidebar({ stats }: { stats: GameStats | null }) {
  ...
  return (
    ...
    <div className="bora-glass-panel rounded-2xl p-6">
    <h3 className="font-display text-lg font-bold text-white mb-6 flex items-center gap-2">
    <span className="material-symbols-outlined text-accent-cyan">bar_chart</span> Performance
                    </h3>
    ...
    <span className="text-sm text-white font-bold">{stats?.total || 0}</span>
    </div>
    ...
    <div className="bora-glass-panel rounded-2xl p-6 relative overflow-hidden flex-grow flex flex-col">
    <div className="absolute inset-0 bg-gradient-to-b from-transparent to-bora-primary/10"></div>
    <div className="flex justify-between items-center mb-4 z-10">
    <h3 className="font-display text-sm font-bold text-white uppercase tracking-wider">Showcase</h3>
    <span className="text-xs text-accent-pink border border-accent-pink/30 px-2 py-0.5 rounded bg-accent-pink/10">{stats?.latest?.card?.rarity || 'NONE'}</span>
    </div>
    <div className="flex-grow flex items-center justify-center py-4 relative z-10 perspective-1000">
    <div className="absolute w-40 h-56 bg-gradient-to-tr from-accent-pink to-bora-primary rounded-xl blur-[30px] opacity-40 animate-pulse"></div>
    <div className="w-48 h-72 bg-gray-900 rounded-xl border border-white/10 relative overflow-hidden shadow-2xl transform hover:scale-105 transition-transform duration-500 group">
    {stats?.latest?.card ? (
        <>
        ...
        <h4 className="font-display font-bold text-xl text-white">{stats.latest.card.member}</h4>
        <p className="text-xs text-gray-300">{stats.latest.card.era}</p>
        </div>
        </>
    ) : (
