...
import { getMasteryBadgeImagePath, getMasteryBadgeRarity, getBadgeRarityColors } from '@/lib/utils/badgeImages'
...
export default function MasteryBadgeRewardsModal({
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'members' | 'eras'>('overview')
    ...
    // Calculate totals
    const totalPossibleBadges = (memberTracks.length + eraTracks.length) * milestones.length
    ...
    const getBadgeStatus = (kind: 'member' | 'era', key: string, milestone: number) => {
        const track = kind === 'member'
    ...
    return (
        <div
            className="fixed inset-0 z-[100] flex items-center justify-center p-2 md:p-4 bg-black/70 backdrop-blur-md animate-fade-in"
            onClick={onClose}
        >
            <div
...
function OverviewTab({
    ...
    const progressPercent = totalPossibleBadges > 0
            ...
            <div className="lg:col-span-8 flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                        {milestones.map((ms) => {
                            ...
                            const imagePath = `/badges/mastery/milestone-${ms.level}.svg`
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className={`w-16 h-16 rounded-full ${colors.bg} flex items-center justify-center overflow-hidden shadow-lg mb-1 p-2 ${colors.glow}`}>
                                        <Image
                                            src={imagePath}
                                    ...
                                    <div className="text-center">
                                        <p className="text-white text-sm font-bold">Level {ms.level}</p>
                                        <p className={`text-[10px] uppercase font-semibold ${colors.text}`}>{ms.badge.rarity}</p>
                                    </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <p className="text-gray-400 text-xs -mt-2">
                        Unique legendary badges for reaching Level 100 mastery with each member
                    </p>
                    <div className="grid grid-cols-4 sm:grid-cols-8 gap-3">
                        {['RM', 'Jin', 'Suga', 'J-Hope', 'Jimin', 'V', 'Jungkook', 'OT7'].map((member) => {
                            const earned = memberBadges.some(b => b.key === member && b.milestone === 100)
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center overflow-hidden p-1 ${earned ? 'bg-yellow-500/20' : 'bg-gray-800 grayscale'}`}>
                                        <Image
                                            src={imagePath}
                                    ...
                                    <p className={`text-[10px] font-bold ${earned ? 'text-white' : 'text-gray-500'}`}>
                                        {member}
                                    </p>
                                    {earned && (
                                        <span className="absolute -top-1 -right-1 material-symbols-outlined text-yellow-400 text-sm">verified</span>
                                    )}
                                </div>
                            )
                        })}
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
                ...
                <div className="bg-[#302249] rounded-2xl p-6 shadow-xl relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col gap-4">
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-gray-400 text-sm font-medium">Total Badges</p>
                ...
                <div className="bg-[#231b2e] border border-[#302249] rounded-2xl p-5 flex flex-col gap-4">
                    <h4 className="text-white font-bold text-sm flex items-center gap-2">
                        <span className="material-symbols-outlined text-purple-400 text-lg">bar_chart</span>
                        Badge Breakdown
                    </h4>
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-[#302249]/50 rounded-xl p-3 text-center">
                            <p className="text-2xl font-bold text-purple-400">{memberBadges.length}</p>
                            <p className="text-[10px] text-gray-400 uppercase">Member Badges</p>
                        </div>
                        <div className="bg-[#302249]/50 rounded-xl p-3 text-center">
                            <p className="text-2xl font-bold text-pink-400">{eraBadges.length}</p>
                ...
                <div className="bg-[#1e2a45] border border-blue-500/20 rounded-2xl p-5 flex gap-4 shadow-lg">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-blue-100/70 text-xs leading-relaxed">
                            Earn XP in member and era tracks by answering quiz questions correctly.
                            When you reach milestone levels (5, 10, 25, 50, 100), claim your rewards
                            to unlock exclusive badges and earn Dust!
                        </p>
                    </div>
                </div>
            </div>
...
function TrackBadgesTab({
    ...
    tracks: MasteryTrack[]
    milestones: Milestone[]
    getBadgeStatus: (kind: 'member' | 'era', key: string, milestone: number) => 'earned' | 'claimable' | 'locked'
}) {
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {tracks.map((track) => {
                    const displayName = track.definition.displayName || track.definition.key
                    const earnedCount = milestones.filter(ms =>
                        getBadgeStatus(kind, track.track.key, ms.level) === 'earned'
                    ).length
                        ...
                        >
                            ...
                            <div className="flex gap-2">
                                {milestones.map((ms) => {
                                    ...
                                    const imagePath = ms.level === 100 && kind === 'member'
                                        ? getMasteryBadgeImagePath(kind, track.track.key, ms.level)
                                        : `/badges/mastery/milestone-${ms.level}.svg`
                                        ...
                                        >
                                            <Image
                                                ...
                                                onError={(e) => {
                                                    ...
                                                    const parent = target.parentElement
                                                    if (parent) {
                                                        const color = status === 'earned' ? 'text-purple-400' : status === 'claimable' ? 'text-yellow-400' : 'text-gray-600'
                                                        parent.innerHTML = `<span class="material-symbols-outlined text-sm ${color}">workspace_premium</span>`
                                                    }
                                                }}
                                            />
                                            {status === 'earned' && (
                                                <span className="absolute -top-1 -right-1 material-symbols-outlined text-green-400 text-xs bg-[#231b2e] rounded-full">check_circle</span>
                                            )}
                                            {status === 'claimable' && (
                                                <span className="absolute -top-1 -right-1 material-symbols-outlined text-yellow-400 text-xs bg-[#231b2e] rounded-full">redeem</span>
                                            )}
                                        </div>
                                    )
                                })}
