  ...
  return (
    <section className="flex-grow flex flex-col gap-6 overflow-hidden h-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
            <div className="bora-glass-panel p-5 rounded-2xl border-l-4 border-l-bora-primary flex items-center justify-between">
                <div>
                    <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Total Collection</p>
                    <h3 className="text-3xl font-display font-bold text-white">{totalCount} <span className="text-base text-gray-500 font-normal">cards</span></h3>
                </div>
                <div className="w-12 h-12 rounded-full bg-bora-primary/10 flex items-center justify-center text-bora-primary">
                    <span className="material-symbols-outlined text-2xl">style</span>
                </div>
            </div>
            <div className="bora-glass-panel p-5 rounded-2xl border-l-4 border-l-accent-cyan flex items-center justify-between">
                <div>
                    <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Unique Cards</p>
                    <h3 className="text-3xl font-display font-bold text-white">{uniqueCount} <span className="text-base text-gray-500 font-normal">/ {totalCount}</span></h3>
                </div>
                <div className="w-12 h-12 rounded-full bg-accent-cyan/10 flex items-center justify-center text-accent-cyan">
                    <span className="material-symbols-outlined text-2xl">auto_awesome</span>
                </div>
            </div>
            <div className="bora-glass-panel p-5 rounded-2xl border-l-4 border-l-accent-pink flex items-center justify-between">
                <div>
                    <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Rare Cards</p>
                    <h3 className="text-3xl font-display font-bold text-white">{rareCount} <span className="text-base text-gray-500 font-normal">found</span></h3>
                </div>
        ...
        <div className="overflow-y-auto pr-2 pb-4 scrollbar-thin flex-grow">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
                {items.map((it) => (
                     <div key={it.id} className="group relative rounded-xl overflow-hidden bora-glass-panel border border-white/5 hover:border-bora-primary/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-bora-primary/10">
                        <div className="relative aspect-[2/3] overflow-hidden">
                            ...
                            <span className={`absolute top-3 right-3 backdrop-blur-sm border text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${
                                it.card?.rarity === 'rare' 
                        ...
                        <div className="absolute bottom-0 left-0 w-full p-4">
                            <div className="flex justify-between items-end">
                                <div>
                                    <h4 className={`font-display font-bold text-white text-lg leading-tight mb-0.5 ${
                ...
                {/* Loading Skeletons */}
                {loading && Array.from({ length: items.length > 0 ? 3 : 10 }).map((_, i) => (
