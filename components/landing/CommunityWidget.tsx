  ...
  return (
    <div className="col-span-1 md:col-span-3 lg:col-span-4 grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
      <div className="lg:col-span-2 glass-panel rounded-2xl p-6">
        <h3 className="text-lg font-bold text-white mb-4">Recent From Blog</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {recentPosts.length > 0 ? (
            recentPosts.map((post) => (
              <Link key={post._id} href={`/blogs/${post._id}`} className="flex gap-4 p-3 rounded-xl hover:bg-secondary/40 transition-colors cursor-pointer group">
                <div
                  className="w-20 h-20 rounded-xl bg-gray-700 bg-cover shrink-0"
                  style={{
                ...
                ></div>
                <div>
                  <span className={`text-xs ${getMoodColor(post.mood)} font-bold`}>{getMoodLabel(post)}</span>
                  <h4 className="font-bold text-white group-hover:text-primary transition-colors line-clamp-2">{post.title}</h4>
            ...
            <>
              <div className="flex gap-4 p-3 rounded-xl hover:bg-secondary/40 transition-colors cursor-pointer group">
                <div className="w-20 h-20 rounded-xl bg-gray-700 bg-cover shrink-0" style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuB2jiH20zV2_3KKI35iM2sa_6NDqt7uCI1HfEq8EzDDtAwXU7QnOBQ1fWdyHhjz_IO8z2vMfBxr0pRnNn7TDmwgxpwO7NvZT_qdB2PCx-piOF90K-zCETJI7DLCHx2altiA5zmAbsmd0vWGmFZsDSp4k-nZxaD28-NqJlCK0ne122CXzSGZ1mvEtUqGDO-TgOJ6CfWgSVelWNLw2fYthbLKBB_PN6MZpCgTt-ECMRN2lhxfMR8ybFhyQ_TzgHyUgim40OLfnVM1tlI')" }}></div>
                <div>
                  <span className="text-xs text-primary font-bold">Analysis</span>
                  <h4 className="font-bold text-white group-hover:text-primary transition-colors">Decoding the metaphors in Rainy Days</h4>
                  <p className="text-xs text-gray-400 mt-1">2 hours ago</p>
                </div>
              </div>
              <div className="flex gap-4 p-3 rounded-xl hover:bg-secondary/40 transition-colors cursor-pointer group">
                <div className="w-20 h-20 rounded-xl bg-gray-700 bg-cover shrink-0" style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuC3LZg78z45WkVI26i-GOHAagWKnRyu98-0Y0hkgfxVD8NIBZT9JLBrFSYj1nqyJp5AS3PFboY0bN2mRbaSIIvIQEPjUsLRhH-VZ90uv75UUUHbsPdtKsnAtAjLMoc-kVm15SiJ0LQEL72VOiqdzNo4XmJXDYty4owd0m6kyp5Kt7BlzARxGaBMTt3Z-wVc19IjqzPo8vR71MGoU5srlf4LaeMZypQz7c1WH0kVSvFFsemh2YhmgNPaoptM_bj79WWF84AlqZ9JBmg')" }}></div>
                <div>
                  <span className="text-xs text-green-400 font-bold">Guide</span>
                  <h4 className="font-bold text-white group-hover:text-primary transition-colors">How to stream effectively on Stationhead</h4>
      ...
      <div className="glass-panel rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col h-full justify-between">
          <h3 className="text-lg font-bold text-white mb-2">Community Pulse</h3>
          <div className="flex items-center gap-4 my-4">
            <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center animate-pulse">
              <Globe className="text-primary w-6 h-6" />
            </div>
            <div>
              <p className="text-2xl font-bold text-white">8,402</p>
              <p className="text-xs text-gray-400">ARMYs online now</p>
            </div>
          </div>
