export default async function Ticker() {
  ...
  return (
    <div className="col-span-1 md:col-span-3 lg:col-span-4 rounded-xl overflow-hidden bg-gradient-to-r from-primary/20 via-secondary to-primary/20 border border-primary/20 h-10 flex items-center relative">
      <div className="flex items-center px-4 bg-primary text-xs font-bold uppercase tracking-wider h-full shrink-0 z-10 shadow-lg text-white">
        What's New
      </div>
      <div className="marquee-container w-full overflow-hidden whitespace-nowrap">
        <div className="marquee-content inline-block animate-marquee">
          {displayItems.map((item, idx) => (
            <span key={idx} className="mx-8 text-sm font-medium text-white/90">• {item}</span>
          ))}
        </div>
      </div>
    </div>
